...
const envSchema = z.object({
  // Application
  APP_NAME: z.string().default('My Next.js App'),
...
// https://www.prisma.io/docs/guides/performance-and-optimization/connection-management#prism-client-in-long-running-applications
...
**Key decision:** Database setup goes in `core/` because it's universal infrastructure. Individual feature models go in their own slices (defined in `prisma/schema.prisma`).
...
#### Example: `shared/models.ts` and Prisma schema

```typescript
// shared/models.ts - TypeScript types for timestamp fields
...
```prisma
// In your prisma/schema.prisma - Add to all models that need timestamps
model Example {
...
**When to use:** All your database models need timestamps. This is genuinely shared behavior, not feature-specific. With Prisma, you add `createdAt` and `updatedAt` fields to each model in your schema.

#### Example: `shared/schemas.ts`
...
├── exceptions.ts          # Feature-specific exceptions
├── validators.ts          # Feature-specific validation (optional)
├── cache.ts               # Feature-specific caching logic (optional)
├── tasks.ts               # Feature-specific background tasks (optional)
├── __tests__/
│   ├── route.test.ts      # Endpoint tests
│   └── service.test.ts    # Business logic tests
└── README.md              # Feature documentation
```

**Note:** Prisma models are defined in `prisma/schema.prisma`, not in feature directories.

**Not every feature needs every file.** Start with `route.ts`, `service.ts`, and `schemas.ts`. Add others as needed.

**Flow:** API Route → Service → Repository → Database
