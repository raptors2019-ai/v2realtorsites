 ...
 *
 * @param param1 - [Standard description]
 *   Use this to [explain purpose and when to vary it].
 * @param param2 - [Standard description]. Range: 1-100.
 *   - Small values (1-10): [use case]
 *   - Medium values (10-50): [use case]
 *   - Large values (50+): [use case and warnings]
 * @param responseFormat - Control output verbosity and token usage.
...
export async function toolName({
  ...
  param1: string;
  param2?: number;
  responseFormat?: "minimal" | "concise" | "detailed";
}): Promise<string> {
  // Implementation here
}
```
...
**❌ Standard TypeScript JSDoc (Insufficient for Agents):**
 ...
 * @returns String containing the note content.
 * @throws FileNotFoundError if note doesn't exist.
 */
export function readNote(path: string, includeFrontmatter: boolean = true): string {
  // Implementation
}
```
 ...
 *
 * Do NOT use this for:
 * - Finding notes (use obsidianVaultQuery with search instead)
 * - Reading MULTIPLE notes at once (use obsidianVaultQuery with batch mode)
 * - Analyzing note relationships (use obsidianGraphAnalyze instead)
 * - Just checking if a note exists (use responseFormat="minimal")
 *
 ...
 * @param responseFormat - Control output verbosity to save tokens.
 ...
 *
 * @returns Formatted markdown string with note content.
 ...
 * })
 */
export async function obsidianNoteRead({
  path,
  responseFormat = "concise"
}: {
  path: string;
...
/**
 * Do NOT use this for:
 * - Searching for notes (use obsidianVaultQuery instead)
 * - Batch reading (use obsidianVaultQuery with batch mode)
 * - Graph analysis (use obsidianGraphAnalyze instead)
 */
```
 ...
 * await readNote("daily/2025-01-15.md", "minimal")
 *
 * @example
 * // Get project overview before update
 * await readNote("projects/website-redesign.md", "concise")
 */
```
...
```typescript
/**
 * @returns String containing note content.
 */
```
...
```typescript
readNote(path)      // Tool 1
patchNote(path, old, newText)  // Tool 2
updateMetadata(path, metadata)  // Tool 3
